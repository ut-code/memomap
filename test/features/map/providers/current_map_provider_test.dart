...
void main() {
  group('CurrentMapIdNotifier', () {
    ...
    setUp(() {
      ...
      when(
        () => mockSyncService.syncWithServer(),
      ).thenAnswer((_) async => <String, String>{});
      when(
        () => mockSyncService.setCurrentMapId(any()),
      ).thenAnswer((_) async {});
    });

    test(
      'should NOT create duplicate default map when reloading with existing map',
      () async {
        ...
        when(
          () => mockSyncService.getAllMaps(),
        ).thenAnswer((_) async => [existingMap]);
        when(
          () => mockSyncService.getCurrentMapId(),
    ...
    test(
      'should create default when cache is empty and savedMapId is stale',
      () async {
    ...
    test(
      'should NOT create new default when reloading with existing default map and pins',
      () async {
    ...
    test(
      'should create default map via ensureValidMapSelected when maps is empty',
      () async {
        ...
        container.read(mapsProvider.notifier).state = const AsyncValue.data([]);
    ...
    test(
      'should create default map when savedMapId is null (first launch)',
      () async {
