...
void main() {
  group('DrawingNotifier race conditions', () {
    ...
    setUp(() {
      ...
      when(
        () => mockSyncService.getAllDrawings(),
      ).thenAnswer((_) async => <DrawingData>[]);
      when(
        () => mockSyncService.clearIfUserChanged(any()),
      ).thenAnswer((_) async {});
    });
    ...
    ProviderContainer createContainer({
      ...
      return ProviderContainer(
        overrides: [
          ...
          currentMapIdProvider.overrideWith(
            (ref) => _MockCurrentMapIdNotifier(),
          ),
        ],
      );
    }

    test('undo after addPath sync is properly serialized', () async {
      // With async lock:
      ...
      when(
        () => mockSyncService.replaceDrawings(
    ...
    test('addPath during undo causes state overwrite', () async {
      final drawingA = createDrawing('a', testPath1);
      ...
      expect(
        ...
        anyOf(equals({'a', 'b', 'd'}), equals({'a', 'b'})),
        reason:
            'State should be consistent (either undone or with new drawing)',
      );
    });
    ...
    test(
      ...
      () async {
        ...
        expect(state.drawingDataList.map((d) => d.id).toSet(), {'a', 'b', 'c'});
      },
    );

    test('concurrent undo calls should not corrupt stack', () async {
      final drawingA = createDrawing('a', testPath1);
      container = createContainer(initialDrawings: [drawingA]);
      ...
      when(
        ...
        return createDrawing(path == testPath1 ? 'b' : 'c', path);
      });
    ...
    test('undo during redo causes stack inconsistency', () async {
      final drawingA = createDrawing('a', testPath1);
      container = createContainer(initialDrawings: [drawingA]);
    ...
    test('concurrent addPath calls should be serialized', () async {
      container = createContainer();
      await container.read(drawingProvider.future);
...
class _MockCurrentMapIdNotifier extends StateNotifier<String?>
